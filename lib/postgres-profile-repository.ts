...
type ProfileRow = { owner_id: string; subdomain: string; emoji: string; created_at: Date; business_name: string; tagline: string | null; location: string | null; services: string[]; phone: string | null; email: string | null; profile_image_url: string | null; onboarding_completed_at: Date | null; payment_link_url: string | null };
...
async function readPostgresProfile(subdomain: string) {
  const result = await query<ProfileRow>(`select b.owner_user_id owner_id, s.subdomain, s.emoji, s.created_at, b.name business_name, s.tagline, s.location, s.services, s.phone, s.email, s.profile_image_url, s.onboarding_completed_at, b.payment_link_url from site s join business b on b.id=s.business_id where s.subdomain=$1 and s.deleted_at is null`, [subdomain]);
...
export const postgresProfileRepository: ProfileRepository = {
  ...
  async writeProfile(subdomain, profile) {
    ...
    await query(`update business b set name=$2,payment_link_url=$3,updated_at=now() from site s where s.business_id=b.id and s.subdomain=$1 and b.owner_user_id=$4`, [subdomain, profile.businessName || subdomain, profile.paymentLinkUrl ?? null, profile.ownerId]);
  },
