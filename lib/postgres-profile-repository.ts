...
const PROFILE_SELECT = `b.owner_user_id owner_id,s.subdomain,s.emoji,s.created_at,s.sitter_name,s.business_name,s.tagline,s.location,s.services,s.phone,s.email,s.linkedin_url,s.profile_image_url,s.onboarding_completed_at,b.payment_link_url,s.availability_status,s.availability_until,s.years_experience,s.care_capabilities,s.meet_and_greet_expectations,s.cancellation_expectations,s.self_reported_credentials`;
...
export const postgresProfileRepository: ProfileRepository = {
  ...
  async writeProfile(subdomain, profile) {
    await query(`update site s set emoji=$2,sitter_name=$3,business_name=$4,tagline=$5,location=$6,services=$7,phone=$8,email=$9,linkedin_url=$10,profile_image_url=$11,onboarding_completed_at=$12,availability_status=$13,availability_until=$14,years_experience=$15,care_capabilities=$16,meet_and_greet_expectations=$17,cancellation_expectations=$18,self_reported_credentials=$19,updated_at=now() from business b where s.business_id=b.id and s.subdomain=$1 and b.owner_user_id=$20 and s.deleted_at is null`, [subdomain, profile.emoji, profile.sitterName ?? null, profile.businessName ?? null, profile.tagline ?? null, profile.location ?? null, profile.services ?? [], profile.phone ?? null, profile.email ?? null, profile.linkedinUrl ?? null, profile.profileImageUrl ?? null, profile.onboardingCompletedAt ? new Date(profile.onboardingCompletedAt) : null, profile.availabilityStatus ?? 'ACCEPTING', profile.availabilityUntil ?? null, profile.yearsExperience ?? null, profile.careCapabilities ?? [], profile.meetAndGreetExpectations ?? null, profile.cancellationExpectations ?? null, profile.selfReportedCredentials ?? [], profile.ownerId]);
