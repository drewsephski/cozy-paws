...
import { createPaymentRequestAction, markLeadReadAction, saveClientFromLeadAction, updateLeadStatusAction, type PaymentRequestState, type SaveClientState } from '@/app/actions';
...
function SaveClientForm({ leadId, saved }: { leadId: string; saved: boolean }) {
  const [state, action, pending] = useActionState<SaveClientState, FormData>(saveClientFromLeadAction, {});
  if (saved || state.success) return <p className="mt-4 flex items-center gap-2 text-sm font-medium text-emerald-700 dark:text-emerald-300"><Check className="size-4" aria-hidden="true" />{state.success || 'Saved as a client'}</p>;
  return <form action={action} className="mt-4"><input type="hidden" name="leadId" value={leadId} /><button disabled={pending} className="inline-flex h-10 items-center gap-2 rounded-lg border border-input bg-background px-4 font-medium transition-colors hover:bg-accent disabled:opacity-60"><UserRoundPlus className="size-4" aria-hidden="true" />{pending ? 'Saving...' : 'Save as client'}</button>{state.error && <p className="mt-2 text-sm text-destructive">{state.error}</p>}</form>;
}

export function LeadInbox({ sites, leads, conversationMessages, clientLeadIds }: { sites: InboxSite[]; leads: InboxLead[]; conversationMessages: Record<string, ConversationMessage[]>; clientLeadIds: string[] }) {
