...
import { buildSiteFilterOptions, formatInquiryDate, type InboxSite } from './lead-inbox-model';
...
export function LeadInbox({ sites, leads, conversationMessages }: { sites: InboxSite[]; leads: InboxLead[]; conversationMessages: Record<string, ConversationMessage[]> }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="space-y-2">{visible.map((lead) => <article key={lead.id} className={`rounded-2xl border bg-card p-4 shadow-sm ${lead.readAt ? 'border-border' : 'border-emerald-500/50'}`}>
      ...
      {expanded === lead.id && <div className="mt-4 border-t border-border pt-4 text-sm"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><a href={`mailto:${lead.email}`} className="min-w-0 truncate font-medium underline underline-offset-4">{lead.email}</a><div className="flex shrink-0 items-center gap-2"><button type="button" onClick={() => void copyDetails(lead)} className="underline underline-offset-4">{copyState === lead.id ? 'Copied' : 'Copy details'}</button><span className="rounded-full bg-muted px-2 py-0.5 text-xs font-medium">{lead.status || 'NEW'}</span></div></div><dl className="mt-4 grid gap-x-6 gap-y-3 rounded-xl border border-border/70 bg-muted/20 p-4 sm:grid-cols-2 lg:grid-cols-4"><div><dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Service</dt><dd className="mt-1 font-medium">{lead.serviceRequested || 'Not specified'}</dd></div><div><dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Dates</dt><dd className="mt-1 font-medium">{lead.requestedStartDate ? `${formatInquiryDate(lead.requestedStartDate)}${lead.requestedEndDate ? ` – ${formatInquiryDate(lead.requestedEndDate)}` : ''}` : lead.dates || 'Not provided'}</dd></div><div><dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Pets</dt><dd className="mt-1 font-medium">{lead.petCount ? `${lead.petCount} ` : ''}{lead.petTypes?.join(', ') || 'Not specified'}</dd></div><div><dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">ZIP code</dt><dd className="mt-1 font-medium">{lead.postalCode || 'Not provided'}</dd></div></dl><div className="mt-5 rounded-xl bg-muted/30 p-4"><ConversationMessages messages={[{ id: `lead-${lead.id}`, sender: 'CUSTOMER', body: lead.message || lead.serviceRequested || 'Availability request', createdAt: lead.createdAt }, ...(conversationMessages[lead.id] || [])]} />{lead.id in conversationMessages ? <ConversationReplyForm participant="SITTER" leadId={lead.id} /> : <p className="mt-4 text-xs text-muted-foreground">This older request continues by email.</p>}</div>{(lead.status === 'NEW' || !lead.status) && <form action={updateLeadStatusAction} className="mt-4 flex flex-wrap gap-2"><input type="hidden" name="leadId" value={lead.id} /><button name="status" value="QUALIFIED" className="rounded-lg bg-primary px-3 py-2 text-primary-foreground">Qualify inquiry</button><button name="status" value="DECLINED" className="rounded-lg border px-3 py-2">Decline</button><button name="status" value="SPAM" className="rounded-lg border px-3 py-2">Spam</button></form>}{canRequestPayment(lead.status) && <PaymentForm lead={lead} />}</div>}
    </article>)}</div>
