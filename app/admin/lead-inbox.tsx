...
export function LeadInbox({ leads, conversationMessages }: { leads: InboxLead[]; conversationMessages: Record<string, ConversationMessage[]> }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="space-y-2">{visible.map((lead) => <article key={lead.id} className={`rounded-2xl border bg-card p-4 shadow-sm ${lead.readAt ? 'border-border' : 'border-emerald-500/50'}`}>
      ...
      {expanded === lead.id && <div className="mt-4 border-t border-border pt-4 text-sm"><div className="flex flex-wrap gap-3"><a href={`mailto:${lead.email}`} className="font-medium underline underline-offset-4">{lead.email}</a><button type="button" onClick={() => void copyDetails(lead)} className="underline underline-offset-4">{copyState === lead.id ? 'Copied' : 'Copy details'}</button><span className="rounded-full bg-muted px-2 py-0.5 text-xs">{lead.status || 'NEW'}</span></div><p className="mt-3 text-muted-foreground">{lead.serviceRequested || 'Service not specified'} · {[lead.requestedStartDate, lead.requestedEndDate].filter(Boolean).join(' to ') || lead.dates || 'Dates not provided'} · {lead.petCount ? `${lead.petCount} ` : ''}{lead.petTypes?.join(', ') || 'Pets not specified'}{lead.postalCode ? ` · ${lead.postalCode}` : ''}</p><div className="mt-5 rounded-xl bg-muted/30 p-4"><ConversationMessages messages={[{ id: `lead-${lead.id}`, sender: 'CUSTOMER', body: lead.message || lead.serviceRequested || 'Availability request', createdAt: lead.createdAt }, ...(conversationMessages[lead.id] || [])]} />{lead.id in conversationMessages ? <ConversationReplyForm participant="SITTER" leadId={lead.id} /> : <p className="mt-4 text-xs text-muted-foreground">This older request continues by email.</p>}</div>{(lead.status === 'NEW' || !lead.status) && <form action={updateLeadStatusAction} className="mt-4 flex gap-2"><input type="hidden" name="leadId" value={lead.id} /><button name="status" value="QUALIFIED" className="rounded-lg bg-primary px-3 py-2 text-primary-foreground">Qualify inquiry</button><button name="status" value="DECLINED" className="rounded-lg border px-3 py-2">Decline</button><button name="status" value="SPAM" className="rounded-lg border px-3 py-2">Spam</button></form>}{canRequestPayment(lead.status) && <PaymentForm lead={lead} />}</div>}
    </article>)}</div>
