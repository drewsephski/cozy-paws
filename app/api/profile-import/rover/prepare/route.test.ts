...
describe('Rover prepare route', () => {
  ...
  it('streams progress and a bounded review event', async () => {
    const imports = { prepareOwnedReview: vi.fn(async (input: { onProgress?: (stage: 'capture_active') => void }) => {
      ...
      return { attemptId: 'id', subdomain: 'site', canonicalRoverUrl: 'https://www.rover.com/members/jamie/', expectedProfileRevision: 0, current: {}, reviewed: { about: 'Visible' }, confidence: { about: 'high' }, expiresAt: 1 };
