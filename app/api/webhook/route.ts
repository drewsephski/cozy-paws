...
export async function POST(request: NextRequest) {
  ...
  try { await processStripeEvent(event); return NextResponse.json({ received: true }); }
  catch (error) { console.error('Stripe webhook reconciliation failed', { eventId: event.id, eventType: event.type, error: error instanceof Error ? error.message : 'Unknown error' }); return NextResponse.json({ error: 'Webhook reconciliation failed' }, { status: 500 }); }
}
