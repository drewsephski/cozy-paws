...
export function LeadForm({ subdomain }: { subdomain: string }) {
  ...
  return <form action={action} className="mt-5 space-y-3">
    ...
    <label className="block"><span className="mb-1.5 block text-xs font-medium">Dates you need care</span><input name="dates" placeholder="For example, Sept. 12 to 15" className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-ring/40" /></label>
