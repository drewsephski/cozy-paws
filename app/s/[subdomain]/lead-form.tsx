...
export function LeadForm({ subdomain, sitterName }: { subdomain: string; sitterName: string }) {
  ...
  const [mode, setMode] = useState<'message' | 'availability'>('message');
  return <div className={`flex flex-col ${mode === 'availability' ? 'min-h-[31rem]' : 'min-h-[24rem]'}`}>
    {state.success ? <LeadSubmissionConfirmation sitterName={sitterName} state={state} /> : <>
      <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-[.16em] text-emerald-700 dark:text-emerald-400">Contact {sitterName}</p>
      <h2 className="text-xl font-semibold">{mode === 'message' ? `Message ${sitterName}` : 'Tell me about your pet'}</h2>
      <p className="mt-1.5 text-sm leading-5 text-muted-foreground">{mode === 'message' ? 'Start a private conversation. I’ll reply by email.' : 'Share your dates and care details. I’ll reply by email.'}</p>
      <div className="mt-4 grid grid-cols-2 gap-2 rounded-xl bg-muted p-1" role="group" aria-label="Choose how to contact the sitter">
        <button type="button" aria-pressed={mode === 'message'} onClick={() => setMode('message')} className={`inline-flex h-9 items-center justify-center gap-2 rounded-lg px-3 text-xs font-semibold outline-none transition focus-visible:ring-2 focus-visible:ring-ring/50 ${mode === 'message' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}><MessageCircle className="size-4" aria-hidden="true" />Direct message</button>
        <button type="button" aria-pressed={mode === 'availability'} onClick={() => setMode('availability')} className={`inline-flex h-9 items-center justify-center gap-2 rounded-lg px-3 text-xs font-semibold outline-none transition focus-visible:ring-2 focus-visible:ring-ring/50 ${mode === 'availability' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}><CalendarDays className="size-4" aria-hidden="true" />Availability details</button>
      </div>
      <form action={action} className="mt-4 space-y-2.5">
    <input type="hidden" name="subdomain" value={subdomain} />
    <input type="hidden" name="source" value={mode === 'message' ? 'sitterfolio_direct_message' : 'sitterfolio_site'} />
    ...
    <label className="block"><span className="mb-1 block text-xs font-medium">{mode === 'message' ? 'Message' : 'Pet and care details'}</span><textarea name="details" required={mode === 'message'} placeholder={mode === 'message' ? `What would you like to ask ${sitterName}?` : 'Pet, routine, and care needed'} rows={mode === 'message' ? 4 : 2} className="w-full resize-none rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring/40" /></label>
    {state.error && <p role="alert" className="text-sm text-destructive">{state.error}</p>}
        <button disabled={pending} className="h-10 w-full rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground transition hover:bg-primary/90 disabled:cursor-wait disabled:opacity-60">{pending ? 'Sending...' : mode === 'message' ? `Message ${sitterName}` : 'Send availability request'}</button>
      </form>
    </>}
  </div>;
}
